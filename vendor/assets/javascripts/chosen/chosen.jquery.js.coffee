# WARNING:
# When updating Chosen from github, note that we made several changes to this file, that need to be applied after
# the upgrade, check the git history of this file to see what was changed.
...
class Chosen extends AbstractChosen
  ...
  set_up_html: ->
    ...
    @f_width = 940 # Set the width to be the same as the page
    ...
    container_div = ($ "<div />", {
      ...
      style: 'width: ' + (@form_field_jq.outerWidth()) + 'px;' #use parens around @f_width so coffeescript doesn't think + ' px' is a function parameter
    })
  ...
  results_show: ->
    ...
    @dropdown.css {"top":  dd_top + "px", "left":-127} # Set the left position of the drop-down such it takes the entire page
  ...
  winnow_results: ->
    ...
    # Ali: replace all multiple spaces with one space, otherwise highlighting breaks
    searchText = searchText.replace(/\ +(?= )/g,'');

    regex = new RegExp(searchText.replace(/[-[\]{}()*+?.,\\^$|#\s]/g, "\\$&").split('\\ ').join('.*'), 'i')
    # Ali: this new zregex is used to highlight matches
    zregex = new RegExp(searchText.replace(/[-[\]{}()*+?.,\\^$|#\s]/g, "\\$&").split('\\ ').join('|'), 'gi')
      ...
      if not option.disabled and not option.empty
        if option.group
          ...
          if regex.test option.html
            found = true
            results += 1
# Ali: commented out for efficiency, we don't need the following code as our regex matches all words
...
# Ali: this is what the original version of Chosen used, it only highlighted one word.
#              startpos = option.html.search regex
#              text = option.html.substr(0, startpos + searchText.length) + '</em>' + option.html.substr(startpos + searchText.length)
#              text = text.substr(0, startpos) + '<em>' + text.substr(startpos)
# Ali: the following line is what we changed to make it highlight all matched words.
               text = option.html.replace(zregex, "<em>$&</em>")
            else
              text = option.html
